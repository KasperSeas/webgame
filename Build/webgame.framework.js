...
var ASM_CONSTS = {
  15849020: () => { Module['emscripten_get_now_backup'] = performance.now; },  
 15849075: ($0) => { performance.now = function() { return $0; }; },  
 15849123: ($0) => { performance.now = function() { return $0; }; },  
 15849171: () => { performance.now = Module['emscripten_get_now_backup']; }
};
...
// end include: preamble.js
  ...
  function _AVPPlayerClose(playerIndex)
  	{
  		...
  		if (_players[playerIndex].ivsjs != null)
  		{
  			_players[playerIndex].ivsjs.destroy();
  			_players[playerIndex].ivsjs = null;
  		}
  ...
  function _AVPPlayerCreateVideoTexture(textureId)
  	{
  		// Check if WebGL build and GLctx is defined
  		if (typeof GLctx === 'undefined' || GLctx === null) {
  			console.error('WebGL context (GLctx) is not initialized.');
  			return;
  		}
  		...
  		try {
  			...
  			console.log('Successfully created and initialized WebGL texture with ID:', textureId);
  		} catch (error) {
  			console.error('Error in AVPPlayerCreateVideoTexture:', error);
  		}
  	}
  ...
  function _AVPPlayerCreateVideoTexture_WPGU(playerIndex)
  	{
  		const device = window.gpuDevice;
  
  		const video = _players[playerIndex].video;
  
  		const textureWidth = video.videoWidth;
  		const textureHeight = video.videoHeight;
  
  		var descriptor = {
  			label: "Created_Texture_Test",
  			size: [textureWidth, textureHeight],
  			format: 'rgba8unorm',
  			usage: GPUTextureUsage.COPY_SRC | GPUTextureUsage.COPY_DST | GPUTextureUsage.TEXTURE_BINDING | GPUTextureUsage.RENDER_ATTACHMENT |  GPUTextureUsage.STORAGE_BINDING
  		};
  		...
  		//const bindGroupPtr = wgpuStoreAndSetParent(bindGroup, device);
  		const texturePtr = wgpuStoreAndSetParent(texture, device);
  
  		console.log(`Created texture: ${texture}; texturePtr: ${texturePtr}`, device);
  ...
  function _AVPPlayerDestroyVideoTexture(textureId)
  	{
  		console.log(`AVPPlayerDestroyVideoTexture. textureId: ${textureId}; `);
  		...
  		{
  			if (!wgpu[textureId]) {
  				console.warn('Texture does not exist for ID:', textureId);
  				return;
  			}
  		
  			try {
  				delete wgpu[textureId];  // Let garbage collection handle the texture destruction
  				console.log('Successfully deleted WebGPU texture with ID:', textureId);
  			} catch (error) {
  				console.error('Error in AVPPlayerDestroyVideoTexture:', error);
  			}
  		}
  		else 
  		{
  			...
  			try {
  				...
  				// Remove the texture from GL.textures
  				delete GL.textures[textureId];
  				
  				console.log('Successfully deleted texture with ID:', textureId);
  			} catch (error) {
  				console.error('Error in AVPPlayerDestroyVideoTexture:', error);
  			}
  		}
  ...
  function _AVPPlayerFetchVideoTexture_WGPU(playerIndex, textureId, init) {
  		const webGPU = typeof navigator.gpu !== "undefined" && navigator.gpu !== null;
          if (!webGPU) {
          ...
          const videoElement = _players[playerIndex].video;
          if (!videoElement) {
              console.error("Video element does not exist for player:", playerIndex);
              return;
          }
  
  		if (videoElement.readyState < HTMLMediaElement.HAVE_CURRENT_DATA) {
  			console.warn("Video is not ready. ReadyState:", videoElement.readyState);
  			return;
  		}
  		
  		if (videoElement.paused || videoElement.ended) {
  			console.warn("Video is not playing.", " Video dimensions:", videoElement.videoWidth, "x", videoElement.videoHeight);
  			return;
  		}
  		...
  		const device = window.gpuDevice;
  		const texture = wgpu[textureId];
  		
  		if (videoElement.videoWidth !== texture.width || videoElement.videoHeight !== texture.height) {
  			console.error("Video dimensions dont match created texture:", videoElement.videoWidth, "x", videoElement.videoHeight);
  			return;
  		}
  		...
  		try {
              ...
              console.error("Error in AVPPlayerFetchVideoTexture:", error);
          }
  	}
  ...
  function _AVPPlayerFetchVideoTexture(playerIndex, textureId, init)
  	{
  		...
  		if(webGPU){
  			_AVPPlayerFetchVideoTexture_WGPU(playerIndex, textureId, init);
  		}
  		else
  		{
  			const webGL = typeof GLctx !== 'undefined' && GLctx !== null;
  			if(!webGL){ return; }
  			...
  			if (!GL.textures[textureId]) {
  				console.error('Texture does not exist for ID:', textureId);
  				return;
  			}
  
  			// Check if the video element exists
  			if (!_players[playerIndex].video) {
  				console.error('Video element does not exist for player:', playerIndex);
  				return;
  			}
  ...
  function _initializeWebGPU() {
  		...
  		try {
  			// official way to get device but doesnt actually work at this time
  			//{
  			...
  			//}
  			var totalMemory = window.navigator.deviceMemory || 4;
  			console.log(`TotalMemory: ${totalMemory}GB`);
  
  			const devicePtr = 4; // TODO: search for id instead of hardcode
  			const device = wgpu[devicePtr];
  			window.gpuDevice = device;
  
  			console.log(`GPUDevice: `, device);
  
  			if(device === null || !(device instanceof GPUDevice)) {
  				console.error('Device is not a WebGPU Device! point is probably not hardcoded 4 anymore :(. Device object=', device);
  			}
  ...
  function _IsDocumentInteracted()
  	{
  		return new Promise((resolve) => {
  			// If the document has already been interacted with, resolve immediately
  			if (_isInteracted) {
  				resolve(true);
  				return;
  			}
  			...
  			function onUserInteraction() {
  				console.log("User has interacted with the document.");
  	
  				_isInteracted = true; // Mark interaction as complete
  	
  				// Resolve the promise for the current call
  				resolve(true);
  ...
  function _AVPPlayerInsertVideoElement(path, idValues, externalLibrary)
  	{
  		_IsDocumentInteracted().then(() => {
  			console.log("Document Interaction detected.");
  		});
  
  		const webGPU = typeof navigator.gpu !== 'undefined' && navigator.gpu !== null;
  		if(webGPU && !window.webGPUInitialized)
  		{
  		...
  		if (externalLibrary == 1) // DASH
  		{
  			useNativeSrcPath = false;
  			var player = dashjs.MediaPlayer().create();
  			player.initialize(vid, path, true);
  		}
  		else if (externalLibrary == 2 && !(_is_iOS() || _isSafari())) // HLS
  		{
  			useNativeSrcPath = false;
  		...
  		else if (externalLibrary == 3) // IVS
  		{
  			useNativeSrcPath = false;
  			vid.srcObject = new MediaStream();
  			// IVS Library: https://web-broadcast.live-video.net/1.15.0/amazon-ivs-web-broadcast.js
  
  			console.log("AVPPlayerInsertVideoElement: Creating ivs stage...");
  
  			let stageToken = "";
  			let publisherParticipantId = "";
  			
  			// Example url path: https://path?stageToken=eyJhbGciOiJLTVMiLCJ0eXAiOiJKV1QifQ&participantId=TsoiOWILYGlv
  			try{
  				if(path && path !== ""){
  					...
  					publisherParticipantId = params.participantId;
  					
  					ivs = new IVSStage(stageToken, publisherParticipantId, vid);
  					ivs.joinStage();
  				}
  			}
  			catch(err){
  		...
  		{
  			console.log("VID oncanplay. hasSetCanPlay=" + hasSetCanPlay);
  		...
  		vid.onloadedmetadata = function()
  		{
  			...
  			console.log("VID hasMetadata");
  		};
  
  		vid.oncanplaythrough = function()
  		{
  			vidData.buffering = false;
  			console.log("VID buffering");
  		};
  		...
  		vid.onwaiting = function()
  		{
  			...
  			console.log("VID WAITING");
  		};
  
  		vid.onstalled = function()
  		{
  			vidData.isStalled = true;
  			console.log("VID STALLED");
  		}
  
  		vid.onpause = function() {
  			console.log("VID ON PAUSE");
  		};
  ...
  function _AVPPlayerPlay(playerIndex)
  	{
  		...
  		if (playPromise !== undefined)
  		{
  			...
  			{
  				console.log(`AVPPlayerPlay promised`);
  				// Automatic playback started!
  				// Show playing UI.
  			})
  			.catch(function(error) 
  			{
  				console.error(`AVPPlayerPlay error: `, error);
  				_IsDocumentInteracted().then(() => {
  					console.log("Interaction detected from call 2.");
  					_players[playerIndex].video.play();
  				});
  ...
  function _AVPPlayerSupportedDecodedFrameCount(playerIndex)
  	{
  		console.error("AVPPlayerSupportedDecodedFrameCount");
  		if (!_hasPlayer(playerIndex)) {	return false; }
  ...
  function _BrowserLockCursor(lockCursor) {
          if (Module.canvas) {
              if (lockCursor) {
  ...
  function _Connect(addressPtr, openCallbackPtr, closeCallBackPtr, messageCallbackPtr, errorCallbackPtr)
  {
   ...
   webSocket.addEventListener("open", function(event) {
    ...
    getWasmTableEntry(openCallbackPtr)(index);
   });
   webSocket.addEventListener("close", function(event) {
    console.log("Disconnected from " + address);
    getWasmTableEntry(closeCallBackPtr)(index);
   });
   webSocket.addEventListener("message", function(event) {
      try {
          if (event.data instanceof ArrayBuffer) {
              ...
              if (bufferPtr === 0) {
                  console.error("[SimpleWeb Error] Memory allocation failed for WebSocket message");
                  return;
              }
      
              if (bufferPtr + arrayLength <= HEAPU8.buffer.byteLength) {
                  var dataBuffer = new Uint8Array(HEAPU8.buffer, bufferPtr, arrayLength);
                  dataBuffer.set(array);
              } else {
                  console.error("[SimpleWeb Error] Invalid bufferPtr or arrayLength exceeds buffer size");
                  return;
              }
              getWasmTableEntry(messageCallbackPtr)(index, bufferPtr, arrayLength);
              _free(bufferPtr);
          } else {
          ...
          console.error("[SimpleWeb Error] Connect message error: ", error);
      }
      
   });
   webSocket.addEventListener("error", function(event) {
    console.error("Socket Error", event);
    getWasmTableEntry(errorCallbackPtr)(index);
   });
   return index;
  }
  ...
  function _getRemoteUserVolume(uid) {}

  
  
  function _getRemoteUserVolume_WGL(uid) {
      var uid_Str = UTF8ToString(uid);
      return getRemoteUserVolume_WGL(uid_Str);
    }
  ...
  function _initEventOnEngineCallback(OnJoinChannelSuccessCallback,
      ...
      UnityHooks.InvokeVideoSizeChangedCallback = function(uid, width, height) {
        Runtime.dynCall('viiiii', UnityHooks.OnVideoSizeChangedCallback, [uid, width, height, 0]);
        //Module['dynCall_viiiii'](UnityHooks.OnStreamMessageCallback, uid, 0, data, length);
      };
  ...
  function _setDistanceUnit(unit) {
      console.log("[Agora] setDistanceUnit 1: "+unit);
  ...
  function _setRemoteUserVolume_WGL(uid, volume) {
      var uid_Str = UTF8ToString(uid);
      setRemoteUserVolume_WGL(uid_Str, volume);
    }
...
var wasmImports = {
  ...
  "AVPPlayerCreateVideoTexture_WPGU": _AVPPlayerCreateVideoTexture_WPGU,
  ...
  "BrowserLockCursor": _BrowserLockCursor,
  "Connect": _Connect,
  ...
  "getRemoteUserVolume": _getRemoteUserVolume,
  "getRemoteUserVolume_WGL": _getRemoteUserVolume_WGL,
  ...
  "setRemoteUserVolume_WGL": _setRemoteUserVolume_WGL,
